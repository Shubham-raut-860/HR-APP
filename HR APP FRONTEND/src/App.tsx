...
import { isTechnicalAdmin } from "@/lib/technicalAdmin";
...
const TechnicalLogin = lazy(() => import("@/pages/TechnicalLogin"));
const PublicJobShare = lazy(() => import("@/pages/PublicJobShare"));
...
const A2AConsole = lazy(() => import("@/pages/A2AConsole"));
const Settings = lazy(() => import("@/pages/Settings"));
const CandidateDashboard = lazy(() => import("@/pages/CandidateDashboard"));
const CandidateCoach = lazy(() => import("@/pages/CandidateCoach"));
...
const CandidateKycUpload = lazy(() => import("@/pages/CandidateKycUpload"));
...
function TechnicalAdminRoute({ children }: { children: React.ReactNode }) {
  const { user, loading, isAuthenticated } = useAuth();
  if (loading) return <AuthLoader />;
  if (!isAuthenticated) return <Navigate to="/technical-login" />;
  if (!isTechnicalAdmin(user)) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-3 text-center px-4">
        <p className="text-lg font-semibold">Technical admin access required</p>
        <p className="text-muted-foreground text-sm max-w-sm">
          This developer console is only available to the dedicated technical admin account.
        </p>
        <Link to="/dashboard" className="mt-2 rounded-full bg-primary px-5 py-2 text-sm font-medium text-primary-foreground">
          Go to dashboard
        </Link>
      </div>
    );
    ...
    <RouteErrorBoundary>
      <Suspense fallback={<TopProgressBar />}>
        <Routes>
          ...
          <Route path="/public/jobs/:id" element={<PublicJobShare />} />
          <Route path="/take-quiz" element={<TakeQuizRoute />} />
          <Route path="/candidate/kyc-upload" element={<CandidateKycUpload />} />
          ...
          <Route path="/copilot" element={<ProtectedRoute allowedRoles={["hr", "admin"]}><DashboardLayout><RecruiterCopilot /></DashboardLayout></ProtectedRoute>} />
          ...
          <Route path="/inspector" element={<Navigate to="/developer/inspector" replace />} />
          <Route path="/a2a" element={<Navigate to="/developer/a2a" replace />} />
          <Route path="/developer/inspector" element={<TechnicalAdminRoute><DashboardLayout><Inspector /></DashboardLayout></TechnicalAdminRoute>} />
          <Route path="/developer/a2a" element={<TechnicalAdminRoute><DashboardLayout><A2AConsole /></DashboardLayout></TechnicalAdminRoute>} />
          <Route path="/settings" element={<ProtectedRoute allowedRoles={["hr", "admin"]}><DashboardLayout><Settings /></DashboardLayout></ProtectedRoute>} />
          <Route path="/notifications" element={<ProtectedRoute allowedRoles={["hr", "admin"]}><DashboardLayout><NotificationsCenter /></DashboardLayout></ProtectedRoute>} />
          ...
          <Route path="/candidate/coach" element={<ProtectedRoute allowedRoles={["candidate"]}><CandidateLayout><CandidateCoach /></CandidateLayout></ProtectedRoute>} />
          ...
          <Route path="/candidate/notifications" element={<ProtectedRoute allowedRoles={["candidate"]}><CandidateLayout><NotificationsCenter /></CandidateLayout></ProtectedRoute>} />
