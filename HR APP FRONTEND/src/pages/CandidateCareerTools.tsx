...
import {
  ...
  FileUp, Palette, ArrowRight, Zap, ChevronRight, ChevronLeft, Upload,
  Eye, X,
} from "lucide-react";
            ...
            {TABS.map(t => {
              ...
              return (
                ...
                >
                  ...
                  {t.label}
                  {t.badge && (
                    <span className={`text-[9px] font-bold px-1 rounded-full ${active ? "bg-white/20" : "bg-primary/10 text-primary"}`}>
                      {t.badge}
                    </span>
                  )}
                </button>
...
function StepDot({ idx, current, label, icon: Icon }: { idx: number; current: number; label: string; icon: any }) {
...
function StepConnector({ done }: { done: boolean }) {
  return (
    <div className={`flex-1 h-0.5 mt-4 rounded-full transition-colors ${done ? "bg-emerald-400" : "bg-muted"}`} />
  );
}
...
function ResumeBuilderTool() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-0">
        {STEPS.map((s, idx) => (
          <React.Fragment key={s.id}>
            <button className="flex-1" onClick={() => setStep(idx)}>
              <StepDot idx={idx} current={step} label={s.label} icon={s.icon} />
            </button>
            {idx < STEPS.length - 1 && <StepConnector done={idx < step} />}
          </React.Fragment>
        ))}
      </div>

      {/* Import banner (always visible on first step) */}
      {step === 0 && (
        <Button
          type="button"
          variant="ghost"
          aria-label="Import resume file to auto-fill builder"
          onClick={() => importRef.current?.click()}
          className="group flex items-center gap-4 px-5 py-4 rounded-xl border-2 border-dashed border-primary/40 bg-primary/10 hover:bg-primary/15 hover:border-primary/60 cursor-pointer transition-all shadow-sm hover:shadow-md"
        >
          <div className="p-2 bg-primary/15 rounded-lg shrink-0 transition-transform group-hover:scale-105">
            {importLoading ? <Loader2 className="h-5 w-5 text-primary animate-spin" /> : <FileUp className="h-5 w-5 text-primary" />}
          </div>
          <div className="flex-1">
            <p className="font-semibold text-sm">Already have a resume?</p>
            <p className="text-xs text-muted-foreground">Upload PDF, DOC, DOCX or image to auto-fill every section instantly</p>
          </div>
      ...
      {/* Target role (steps 0-5) */}
      {step < 6 && (
      <div className="flex gap-3 items-center">
        <div className="flex-1">
          <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block mb-1.5">Target Role</label>
          <Input placeholder="e.g. Senior Software Engineer at a startup" value={targetRole} onChange={e => setTargetRole(e.target.value)} className="max-w-md" />
        </div>
        {step === 2 && (
          <div className="flex-1">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block mb-1.5">
              Job ID (for AI bullet enhance)
            </label>
            <Input
              ...
              className="max-w-md"
            />
            {!isValidJobId(bulletEnhanceJobId) && (
              <p className="text-[11px] text-muted-foreground mt-1">
                AI bullet enhancement is disabled until a valid Job ID is provided.
              </p>
            )}
          </div>
        )}
                    ...
                    <div className="space-y-2">
                      ...
                      {entry.bullets.map((bullet, bi) => (
                        <div key={bi} className="flex gap-2 items-start">
                          ...
                          />
                          <button title="AI enhance" onClick={() => handleEnhanceBullet(i, bi)} disabled={!bullet.trim() || enhancingBullet !== null || !isValidJobId(bulletEnhanceJobId)}
      ...
      {showPreview && builtResume && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          />
          <div className="relative z-10 bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-auto">
            <div className="flex items-center justify-between px-5 py-3 border-b sticky top-0 bg-white rounded-t-2xl">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: currentTheme.color }} />
                <span className="font-semibold text-sm">{currentTheme.label} Theme Preview</span>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className={`p-8 space-y-5 text-zinc-900 text-sm ${currentTheme.font === 'serif' ? 'font-serif' : 'font-sans'}`}>
              {builtResume.contact && (
