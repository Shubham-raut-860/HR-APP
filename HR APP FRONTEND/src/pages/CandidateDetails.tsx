...
import {
  ...
  downloadResume,
  draftCandidateEmail,
  createCandidateKycInvite,
  listCandidateKycDocuments,
  setHireApproval,
  sendCandidateEmail,
...
export default function CandidateDetails() {
  ...
  const [hireApproved, setHireApprovedState] = useState(false);
  const [updatingHireApproval, setUpdatingHireApproval] = useState(false);
  const [kycDocs, setKycDocs] = useState<any[]>([]);
  const [kycLoading, setKycLoading] = useState(false);
  const [kycError, setKycError] = useState<string | null>(null);
  const [sendingKycInvite, setSendingKycInvite] = useState(false);
  const [latestKycLink, setLatestKycLink] = useState<string>("");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setCandidate(data);
        if ((data?.tag || "").toLowerCase() === "strong" || (data?.tag || "").toLowerCase() === "medium") {
          await loadKycDocs(data.id);
        } else {
          setKycDocs([]);
          setKycError("Candidate must be shortlisted before KYC access can be requested.");
        }
        if (data.job_id) {
  ...
  const applyEmailTemplate = (key: EmailTemplateKey) => {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      if (interviewAt) {
        ...
        interviewIso = parsed.toISOString();
        scheduleLines.push(`Interview time: ${parsed.toLocaleString()}`);
      }
      if (meetingLink.trim()) scheduleLines.push(`Meeting link: ${meetingLink.trim()}`);
      if (interviewNote.trim()) scheduleLines.push(`Note: ${interviewNote.trim()}`);
      const outboundBody = scheduleLines.length
        ? `${emailBody.trim()}\n\nInterview details:\n${scheduleLines.join("\n")}`
        : emailBody.trim();

      await sendCandidateEmail(candidate.id, emailSubject, outboundBody, {
        interview_at: interviewIso,
        meeting_link: meetingLink.trim() || undefined,
        interview_note: interviewNote.trim() || undefined,
      });
  ...
  const loadKycDocs = async (candidateId: string) => {
    setKycLoading(true);
    setKycError(null);
    try {
      const docs = await listCandidateKycDocuments(candidateId);
      setKycDocs(Array.isArray(docs) ? docs : []);
      setHireApprovedState(true);
    } catch (err: any) {
      setKycDocs([]);
      setKycError(err?.response?.data?.detail || "KYC documents are not accessible yet.");
    } finally {
      setKycLoading(false);
    }
  };

  const handleHireApproval = async (approved: boolean) => {
    if (!candidate?.id) return;
    setUpdatingHireApproval(true);
    try {
      const res = await setHireApproval(candidate.id, approved);
      setHireApprovedState(!!res.approved);
      toast.success(approved ? "Candidate marked approved to hire" : "Hire approval revoked");
      await loadKycDocs(candidate.id);
    } catch (err: any) {
      toast.error(err?.response?.data?.detail || "Failed to update hire approval");
    } finally {
      setUpdatingHireApproval(false);
    }
  };

  const handleSendKycInvite = async () => {
    if (!candidate?.id) return;
    setSendingKycInvite(true);
    try {
      const res = await createCandidateKycInvite(candidate.id, {
        expires_minutes: 45,
        retention_days: 60,
        require_masked_aadhaar: true,
        legal_hold_required: false,
      });
      setLatestKycLink(res.upload_url || "");
      if (res.upload_url && navigator?.clipboard?.writeText) {
        try {
          await navigator.clipboard.writeText(res.upload_url);
          toast.success("Secure KYC link created and copied");
        } catch {
          toast.success("Secure KYC link created");
        }
      } else {
        toast.success("Secure KYC link created");
      }
    } catch (err: any) {
      toast.error(err?.response?.data?.detail || "Failed to create secure KYC link");
    } finally {
      setSendingKycInvite(false);
    }
  };
  ...
  const isShortlisted = (tag || "").toLowerCase() === "strong" || (tag || "").toLowerCase() === "medium";
  const tagStyle = TAG_STYLES[tag];
  const resumeScore = Math.round(candidate.resume_score ?? 0);
  const applicationTimeline = [
    { key: "applied", label: "Applied", done: true, at: candidate.created_at },
    { key: "resume", label: "Resume Evaluated", done: candidate.resume_score != null, at: candidate.updated_at ?? candidate.created_at },
    { key: "shortlist", label: "Shortlisted", done: isShortlisted, at: isShortlisted ? (candidate.updated_at ?? candidate.created_at) : null },
    { key: "quiz", label: "Quiz Evaluated", done: candidate.quiz_score != null, at: candidate.quiz_score != null ? (candidate.updated_at ?? candidate.created_at) : null },
    { key: "decision", label: "Final Decision", done: candidate.final_score != null || candidate.passed != null, at: (candidate.final_score != null || candidate.passed != null) ? (candidate.updated_at ?? candidate.created_at) : null },
  ];
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-0 pb-16">
      ...
      <div className="sticky top-16 z-20 mb-6 rounded-2xl border bg-background/95 p-3 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-background/80">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-center gap-3">
            <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-sm font-bold ${scoreBg(resumeScore)} ${scoreText(resumeScore)}`}>
              {(candidate.name || "?")[0].toUpperCase()}
            </div>
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold">{candidate.name || "Unknown Candidate"}</p>
              <div className="mt-1 flex flex-wrap items-center gap-2">
                {tag && tagStyle && <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${tagStyle.badge}`}>{tag}</span>}
                <span className="text-xs text-muted-foreground">Resume {resumeScore}%</span>
                {job?.title && <span className="hidden truncate text-xs text-muted-foreground sm:inline">for {job.title}</span>}
              </div>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            {candidate.email && (
              <Button variant="outline" size="sm" className="gap-1.5 rounded-xl" onClick={handleOpenContact}>
                <Mail className="h-3.5 w-3.5" />
                Email
              </Button>
            )}
            <Button size="sm" variant="outline" onClick={handleDownload} disabled={downloading} className="gap-1.5 rounded-xl">
              <Download className="h-3.5 w-3.5" />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="rounded-2xl border bg-card p-5">
            <SectionHeader icon={Calendar} title="Application Status Timeline" />
            <div className="space-y-3">
              {applicationTimeline.map((step) => (
                <div key={step.key} className="flex items-center justify-between rounded-lg border bg-muted/10 px-3 py-2">
                  <div className="flex items-center gap-2">
                    {step.done ? (
                  ...
                  <span className="text-xs text-muted-foreground">
                    {step.at ? new Date(step.at).toLocaleString("en-GB", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }) : "Pending"}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="rounded-2xl border bg-card p-5">
            <SectionHeader icon={CheckCircle2} title="Consent-Gated KYC Access" />
            {!isShortlisted ? (
              <p className="text-sm text-muted-foreground">
                KYC records stay locked until this candidate is shortlisted.
              </p>
            ) : (
              <div className="space-y-3">
                <div className="flex flex-wrap items-center gap-2">
                  <Badge variant={hireApproved ? "default" : "secondary"}>
                    {hireApproved ? "Approved to hire" : "Not approved to hire"}
                  </Badge>
                  <Button
                    size="sm"
                    variant={hireApproved ? "outline" : "default"}
                    onClick={() => void handleHireApproval(!hireApproved)}
                    disabled={updatingHireApproval}
                  >
                    {updatingHireApproval ? "Saving..." : hireApproved ? "Revoke approval" : "Approve to hire"}
                  </Button>
                  <Button
                    size="sm"
                    variant="default"
                    onClick={() => void handleSendKycInvite()}
                    disabled={!hireApproved || sendingKycInvite}
                  >
                    {sendingKycInvite ? "Creating link..." : "Send secure KYC link"}
                  </Button>
                </div>
                {latestKycLink ? (
                  <p className="text-xs text-muted-foreground break-all">
                    Latest secure link: {latestKycLink}
                  </p>
                ) : null}
                {kycLoading ? (
                  <p className="text-sm text-muted-foreground">Checking KYC access...</p>
                ) : kycError ? (
                  <p className="text-sm text-muted-foreground">{kycError}</p>
                ) : kycDocs.length === 0 ? (
                  <p className="text-sm text-muted-foreground">No KYC documents uploaded yet.</p>
                ) : (
                  <div className="space-y-2">
                    {kycDocs.map((doc) => (
                      <div key={doc.doc_type} className="flex items-center justify-between rounded-lg border bg-muted/10 px-3 py-2">
                        <div>
                          <p className="text-sm font-medium">{String(doc.doc_type).replace("_", " ")}</p>
                          <p className="text-xs text-muted-foreground">
                            Status: {String(doc.status || "").toUpperCase() || "UPLOADED"}
                            {doc.retention_expires_at ? ` | Retention until ${new Date(doc.retention_expires_at).toLocaleDateString("en-GB")}` : ""}
                          </p>
                        </div>
                        <Badge variant="secondary">Status only</Badge>
                      </div>
                    ))}
                  </div>
                )}
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                {EMAIL_TEMPLATES.map((template) => (
                  <button
                    key={template.key}
                    type="button"
                    onClick={() => applyEmailTemplate(template.key)}
            ...
            <div className="rounded-xl border bg-muted/20 p-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-medium">Interview scheduling hook</p>
                  <p className="text-xs text-muted-foreground">Optional details are included in the email and logged for recruiter audit.</p>
                </div>
              </div>
              ...
              <Textarea
                className="mt-3 min-h-[70px] resize-none text-sm"
                placeholder="Optional note, panel name, or preparation instructions"
                value={interviewNote}
