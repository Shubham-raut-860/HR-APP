...
export default function JobDetails() {
  ...
  const [magicLinks, setMagicLinks] = useState<{ name: string, email: string, link: string | null, state?: string, reason?: string }[]>([]);
  ...
  const handleSendQuizToAll = async (quizId: string) => {
    ...
    try {
      ...
      const sentCount = (result.created_count || 0) + (result.rotated_count || 0);
      const skippedCount = result.skipped?.length || 0;
      toast.success(
        skippedCount > 0
          ? `Quiz links queued for ${sentCount}; ${skippedCount} skipped`
          : `Quiz links queued for ${sentCount || shortlisted.length} candidates`
      );

      // Restore the missing trigger for the Magic Links Modal
      const links = result.links || (result as any).magic_links || (Array.isArray(result) ? result : []);
  ...
  const publicJobLink = `${window.location.origin}/public/jobs/${job.id}`;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex gap-2 shrink-0">
          <Button variant="outline" size="sm" onClick={handleCopyPublicJobLink}>
              ...
              <div key={idx} className="flex items-center justify-between p-3 border rounded-lg bg-muted/20">
                <div className="overflow-hidden mr-4">
                  ...
                  {ml.link ? (
                    ...
                    <div className="text-xs bg-amber-50 text-amber-700 border border-amber-200 p-1 rounded truncate dark:bg-amber-950/30 dark:text-amber-300 dark:border-amber-800">
                      Skipped: {ml.reason || "existing assessment cannot be resent"}
                    </div>
                  )}
                </div>
                <Button variant="outline" size="sm" className="shrink-0" disabled={!ml.link} onClick={() => {
                  if (!ml.link) return;
