...
export default function CandidateMockTest() {
  ...
  const [testMode, setTestMode]         = useState<'resume' | 'topic'>('resume');
  ...
  const handleStartTest = async () => {
    ...
    setLoading(true);
    try {
      const mockContext = testMode === 'resume'
      ...
      toast.error(error.response?.data?.detail || "Failed to generate test. Please try again.");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="bg-muted/50 p-1.5 rounded-full flex items-center border border-border">
                  <button
                    onClick={() => setTestMode('resume')}
                    className={cn("flex-1 py-2 text-sm font-medium rounded-full transition-all flex items-center justify-center gap-2",
                      testMode === 'resume' ? 'bg-background shadow-sm text-foreground border border-border' : 'text-muted-foreground hover:text-foreground'
                    )}
                  >
                  ...
                  <button
                    ...
                    className={cn("flex-1 py-2 text-sm font-medium rounded-full transition-all flex items-center justify-center gap-2",
                      testMode === 'topic' ? 'bg-background shadow-sm text-foreground border border-border' : 'text-muted-foreground hover:text-foreground'
                    )}
                  >
                    <BookOpen className="h-4 w-4" /> Specific Topic
                  </button>
                </div>
