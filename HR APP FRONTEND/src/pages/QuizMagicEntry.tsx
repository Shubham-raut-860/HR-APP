...
const isPlausibleQuizToken = (value: string) => /^[A-Za-z0-9_-]{32,}$/.test(value);
...
export default function QuizMagicEntry() {
  ...
  useEffect(() => {
    ...
    if (!isPlausibleQuizToken(token)) {
      clearQuizToken();
      setCtxError('This assessment link is invalid or unavailable.');
      setCtxLoading(false);
      return;
    }
  ...
  useEffect(() => {
    ...
    const run = async () => {
      if (!token) return;
      if (!isPlausibleQuizToken(token)) return;
      setCtxLoading(true);
      setCtxError('');
      try {
