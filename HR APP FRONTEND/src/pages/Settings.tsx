...
type SettingsTab = "profile" | "company" | "account" | "notifications" | "credentials";
...
export default function Settings() {
  ...
  const settingsTabOptions: ReadonlyArray<{ value: SettingsTab; label: string; icon: ElementType }> = [
    { value: "profile", label: "Profile", icon: User },
    { value: "company", label: "Company", icon: Building },
    { value: "account", label: "Account", icon: Shield },
    { value: "notifications", label: "Notifications", icon: Bell },
    { value: "credentials", label: "Credentials", icon: Mail },
  ];
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="settings-full-name" value={profile.name} onChange={(e) => setProfile({ ...profile, name: e.target.value })} placeholder="e.g. Sarah Johnson" className="rounded-xl bg-muted/20" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="settings-email">Email Address</Label>
                  <Input id="settings-email" value={profile.email} disabled className="rounded-xl bg-muted/50 cursor-not-allowed" />
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="settings-company-name" value={profile.company} onChange={(e) => setProfile({ ...profile, company: e.target.value })} placeholder="e.g. Acme Technologies" className="rounded-xl bg-muted/20 font-medium" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="settings-recruiter-role">Your Role</Label>
                  <Input id="settings-recruiter-role" value={profile.role} onChange={(e) => setProfile({ ...profile, role: e.target.value })} placeholder="e.g. Senior Recruiter" className="rounded-xl bg-muted/20" />
                </div>
              </div>
              <div className="space-y-2">
                <Label htmlFor="settings-company-website">Company Website / Blog</Label>
                <div className="relative">
                  <Globe className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                  <Input id="settings-company-website" value={profile.companyBlog} onChange={(e) => setProfile({ ...profile, companyBlog: e.target.value })} placeholder="https://..." className="pl-9 rounded-xl bg-muted/20" />
                </div>
              </div>
              <div className="space-y-2">
                <Label htmlFor="settings-company-bio">About Company (Bio)</Label>
                <Textarea
                  id="settings-company-bio"
            ...
            <Dialog open={isPwdOpen} onOpenChange={(open) => {
              ...
              <DialogContent className="sm:max-w-[420px] rounded-2xl">
                ...
                <div className="space-y-4 py-2">
                  {[
                    { label: "Current Password", key: "current_password", id: "settings-current-password", show: showCurrent, setShow: setShowCurrent },
                    { label: "New Password", key: "new_password", id: "settings-new-password", show: showNew, setShow: setShowNew },
                    { label: "Confirm New Password", key: "confirm_password", id: "settings-confirm-password", show: showConfirm, setShow: setShowConfirm },
                  ].map(({ label, key, id, show, setShow }) => (
                    <div key={key} className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        />
                        <button type="button" aria-label={show ? `Hide ${label.toLowerCase()}` : `Show ${label.toLowerCase()}`} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground" onClick={() => setShow((v: boolean) => !v)}>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  <Label htmlFor="settings-email-alerts">Email Alerts</Label>
                  <p className="text-xs text-muted-foreground">Receive updates on candidates.</p>
                </div>
                <Switch id="settings-email-alerts" checked={profile.notifications} onCheckedChange={(c) => setProfile({ ...profile, notifications: c })} />
              </div>
            </CardContent>
            <CardFooter className="bg-muted/10 border-t justify-end p-4">
              <Button onClick={handleSave} disabled={loading} className="rounded-full px-6">
                ...
                <>
                  <CardContent className="space-y-5">
                    ...
                    <div className="space-y-4">
                      <div className="space-y-2">
                        <Label id="settings-email-provider-label">Email Provider</Label>
                        <Select
                          value={smtpProvider}
                          onValueChange={(val) => {
                            setSmtpProvider(val);
                        ...
                        >
                          <SelectTrigger className="w-full bg-muted/20" aria-labelledby="settings-email-provider-label">
                      ...
                      <div className="grid grid-cols-[1fr_120px] gap-4">
                        <div className="space-y-2">
                          <Label htmlFor="settings-smtp-server" className="flex items-center gap-1.5"><Server className="h-3.5 w-3.5 text-muted-foreground" /> SMTP Server</Label>
                          <Input
                    ...
                    {/* Username */}
                    <div className="space-y-2">
                      <Label htmlFor="settings-smtp-username" className="flex items-center gap-1.5"><Mail className="h-3.5 w-3.5 text-muted-foreground" /> Email / Username</Label>
                      <Input
                    ...
                    {/* Password */}
                    <div className="space-y-2">
                      <Label htmlFor="settings-smtp-password" className="flex items-center gap-1.5"><Lock className="h-3.5 w-3.5 text-muted-foreground" /> Password / App Password</Label>
                      <div className="relative">
                        <Input
                        ...
                        <button
                          type="button"
                          aria-label={showSmtpPwd ? "Hide SMTP password" : "Show SMTP password"}
