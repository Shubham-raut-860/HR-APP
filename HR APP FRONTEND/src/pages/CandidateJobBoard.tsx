...
function CandidateJobBoardContent() {
  ...
  const statusCounts = useMemo(() => {
    const visibleCount = jobs.filter((job) => !hiddenJobs.includes(job.id)).length;
    const appliedCount = jobs.filter((job) => appliedJobs.includes(job.id) && !hiddenJobs.includes(job.id)).length;
  ...
  }, [jobs, hiddenJobs, appliedJobs]);
  const statusOptions = useMemo(
    () => ([
      { value: "all" as const, label: "All", badge: statusCounts.all },
      { value: "applied" as const, label: "Applied", badge: statusCounts.applied },
      { value: "hidden" as const, label: "Hidden", badge: statusCounts.hidden },
    ]),
    [statusCounts.all, statusCounts.applied, statusCounts.hidden],
  );
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      {activeFiltersCount > 0 && (
        <div className="flex flex-wrap items-center gap-2">
          {selectedCompanies.map((value) => (
            <Badge key={`comp-${value}`} variant="secondary" className="gap-1 pl-2 pr-1 py-1">
              {value}
              <button
                type="button"
                aria-label={`Remove ${value} filter`}
                className="rounded-full p-0.5 hover:bg-muted"
                onClick={() => setSelectedCompanies((prev) => prev.filter((item) => item !== value))}
              >
                <X className="h-3 w-3" />
              </button>
            </Badge>
          ...
          {selectedTypes.map((value) => (
            <Badge key={`type-${value}`} variant="secondary" className="gap-1 pl-2 pr-1 py-1">
              {value}
              <button
                type="button"
                aria-label={`Remove ${value} filter`}
                className="rounded-full p-0.5 hover:bg-muted"
                onClick={() => setSelectedTypes((prev) => prev.filter((item) => item !== value))}
              >
                <X className="h-3 w-3" />
              </button>
            </Badge>
          ...
          {selectedLocations.map((value) => (
            <Badge key={`loc-${value}`} variant="secondary" className="gap-1 pl-2 pr-1 py-1">
              {value}
              <button
                type="button"
                aria-label={`Remove ${value} filter`}
                className="rounded-full p-0.5 hover:bg-muted"
                onClick={() => setSelectedLocations((prev) => prev.filter((item) => item !== value))}
              >
                <X className="h-3 w-3" />
              </button>
            </Badge>
          ))}
          {selectedExp.map((value) => (
            <Badge key={`exp-${value}`} variant="secondary" className="gap-1 pl-2 pr-1 py-1">
              {value}
              <button
                type="button"
                aria-label={`Remove ${value} filter`}
                className="rounded-full p-0.5 hover:bg-muted"
                onClick={() => setSelectedExp((prev) => prev.filter((item) => item !== value))}
              >
                <X className="h-3 w-3" />
              </button>
            </Badge>
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
        <Badge variant="outline" className="font-normal">Showing {filteredJobs.length} roles</Badge>
        <Badge variant="outline" className="font-normal">Saved {savedJobs.length}</Badge>
        <Badge variant="outline" className="font-normal">Applied {appliedJobs.length}</Badge>
      </div>
