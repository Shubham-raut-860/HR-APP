...
export default function CandidateApply() {
  ...
  const [loadWaitSeconds, setLoadWaitSeconds] = useState(0);
  ...
  useEffect(() => {
    if (!loading) {
      setLoadWaitSeconds(0);
      return;
    }
    const start = Date.now();
    const timer = window.setInterval(() => {
      setLoadWaitSeconds(Math.floor((Date.now() - start) / 1000));
    }, 1000);
    ...
    return (
      <div className="space-y-6 w-full">
        ...
        <div className="text-center text-sm text-muted-foreground">
          {loadWaitSeconds >= 8
            ? "Still loading this job. You can wait a bit more or go back and retry."
            : "Loading application form..."}
        </div>
        {loadWaitSeconds >= 8 && (
          <div className="flex justify-center gap-2">
            <Button variant="outline" onClick={() => navigate("/candidate/jobs")}>Back to Jobs</Button>
              ...
              {hasApplied ? (
                ...
                <div className="py-8 text-center space-y-4 border-2 border-dashed rounded-lg bg-muted/10">
                  ...
                  <div className="space-y-2 px-6">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Parsing resume, mapping skills to JD, and preparing fit summary.
                    </p>
                  </div>
                </div>
              ) : evaluation ? (
