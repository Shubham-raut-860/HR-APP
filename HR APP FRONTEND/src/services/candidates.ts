...
export interface CandidateHireApproval {
  candidate_id: string;
...
export interface RecruiterCandidateKycDocument {
  doc_type:
    | 'aadhaar'
    | 'pan'
    | 'employment_proof'
...
export interface CandidateKycInviteResponse {
  invite_id: string;
  candidate_id: string;
  recruiter_user_id: string;
  job_id: string;
...
export const setHireApproval = async (candidateId: string, approved: boolean): Promise<CandidateHireApproval> => {
  const response = await api.post(`/resumes/${candidateId}/hire-approval`, { approved });
  return response.data;
};

export const listCandidateKycDocuments = async (candidateId: string): Promise<RecruiterCandidateKycDocument[]> => {
  const response = await api.get(`/resumes/${candidateId}/kyc-documents`);
  return response.data;
};

export const createCandidateKycInvite = async (
  candidateId: string,
  payload?: {
...
): Promise<CandidateKycInviteResponse> => {
  const response = await api.post(`/resumes/${candidateId}/kyc-invite`, payload || {});
  return response.data;
};

export const downloadCandidateKycDocument = async (
  candidateId: string,
  docType:
    | 'aadhaar'
    | 'pan'
  ...
  try {
    const response = await api.get(`/resumes/${candidateId}/kyc-documents/${docType}/download`, {
      responseType: 'blob',
    });
    const blob: Blob = response.data;
...
export const sendCandidateEmail = async (
  ...
  subject: string,
  body: string,
  opts?: { interview_at?: string; meeting_link?: string; interview_note?: string },
) => {
  const response = await api.post(`/resumes/${id}/send-email`, { subject, body, ...(opts || {}) });
  return response.data;
};
...
export const getRecruiterAuditTrail = async (
  opts?: { limit?: number; resource?: string; signal?: AbortSignal }
): Promise<RecruiterAuditLog[]> => {
  const response = await api.get<RecruiterAuditLog[]>('/resumes/audit-trail', {
    params: {
      limit: opts?.limit ?? 20,
