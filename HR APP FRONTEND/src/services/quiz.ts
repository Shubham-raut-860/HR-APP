...
export interface QuizLinkDispatchResult {
  message: string;
  links: Array<{ name: string; email: string; link: string | null; state?: string; reason?: string }>;
  created_count?: number;
  rotated_count?: number;
  skipped?: Array<{ candidate_id: string; reason: string }>;
}

export const sendQuizLinks = async (candidateIds: string[], quizId: string): Promise<QuizLinkDispatchResult> => {
