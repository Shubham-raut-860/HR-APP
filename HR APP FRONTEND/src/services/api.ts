...
export type ApiErrorClassification =
  | { kind: 'expected-auth'; reason: string }
  | { kind: 'expected-validation'; reason: string }
  | { kind: 'expected-abort'; reason: string };

export const classifyApiError = (error: any): ApiErrorClassification | null => {
  if (axios.isCancel(error) || error?.code === 'ERR_CANCELED') {
    return { kind: 'expected-abort', reason: 'request canceled during navigation or component cleanup' };
  }
  ...
  if (status === 401 && requestUrl.includes('/auth/login')) {
    return { kind: 'expected-auth', reason: 'invalid credentials or restricted login attempt' };
  }
  if (status === 400 && requestUrl.includes('/auth/reset-password/verify')) {
    return { kind: 'expected-validation', reason: 'invalid or expired reset-password token' };
  }
  if (status === 422 && requestUrl.includes('/candidate/my-resumes')) {
    return { kind: 'expected-validation', reason: 'resume upload failed validation or parsing checks' };
  }
...
const REFRESH_REUSE_WINDOW_MS = 3000;
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (classification) {
      (error as any).hireaiClassification = classification;
    }
        ...
        if (refreshToken) {
          ...
          try {
            ...
            if (
              ...
              lastRefreshToken &&
              (now - lastRefreshAtMs) <= REFRESH_REUSE_WINDOW_MS
            ) {
            ...
            if (!refreshAccessPromise) {
              refreshAccessPromise = (async () => {
                ...
                if (newAccessToken) {
                  ...
                  lastRefreshToken = newAccessToken;
