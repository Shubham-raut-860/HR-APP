...
function ScoreBadge({ score, tag }: { score: number; tag: string }) {
  ...
  return (
    ...
    <div className="flex items-center gap-2 w-full min-w-0">
      ...
      <span className={cn("text-[10px] font-bold shrink-0 whitespace-nowrap px-1.5 py-0.5 rounded-full",
        tag === "strong" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400" :
        tag === "medium" ? "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400" : "bg-red-100 text-red-600 dark:bg-red-900/40 dark:text-red-400"
...
export default function ResumePickerModal({
          ...
          <DialogHeader className="px-6 pt-6 pb-4 border-b border-border/40 bg-muted/20">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogDescription className="text-xs mt-0.5">{resumes.length > 0 ? "Choose a resume - AI match score shown for each" : "Upload a resume to apply"}
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>

          <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">{/* Server-side error banner */}
            {serverError && (
                    ...
                    {resumes.map(r => {
                      ...
                      return (
                        <Card
                          ...
                          className={cn("group relative flex flex-col gap-2.5 p-3.5 rounded-xl border cursor-pointer transition-all duration-200",
  ...
  <div className={cn("shrink-0 h-4 w-4 rounded-full border-2 flex items-center justify-center transition-all", selected === r.id ? "border-primary bg-primary" : "border-muted-foreground/40")}>{selected === r.id && <div className="h-1.5 w-1.5 rounded-full bg-white" />}
                            </div>
  ...
  <FileText className={cn("h-4 w-4", selected === r.id ? "text-primary" : "text-muted-foreground")} />
                            ...
                            <div className="flex-1 min-w-0 pr-20">{editingId === r.id ? (
                                ...
                                <>
                                  <div className="flex items-center gap-1.5 min-w-0">
                                    <span className="block min-w-0 flex-1 text-sm font-medium truncate" title={r.label}>{r.label}</span>{r.is_default && <Badge className="text-[9px] px-1.5 py-0 h-4 bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 shrink-0">Default</Badge>}
                                  </div>
                          ...
                          </div>{/* Action buttons: absolutely positioned so they never consume flex space in the row */}
                          {editingId !== r.id && (
                            <div className="absolute top-2.5 right-2.5 z-20 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>{!r.is_default && <Button size="icon" variant="ghost" className="h-7 w-7 hover:text-amber-500 rounded-lg bg-background/80 backdrop-blur-sm" title="Set as default" onClick={() => handleSetDefault(r.id)}><Star className="h-3.5 w-3.5" /></Button>}
                          ...
                          {editingId !== r.id && (
                            <div className="relative z-10 pl-10 overflow-hidden">{isFetching ? (
  ...
  <Button type="button" variant="ghost" aria-label="Upload resume file" onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={e => { e.preventDefault(); setDragOver(false); handleFiles(e.dataTransfer.files); }} onClick={() => fileRef.current?.click()} className={cn("w-full h-auto border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all", dragOver ? "border-primary bg-primary/10" : "border-border/50 hover:border-primary/50 hover:bg-muted/30")}>
          ...
          <div className="px-6 py-4 border-t border-border/40 bg-muted/10 flex items-center justify-between gap-3">
            <div className="text-xs text-muted-foreground min-w-0 flex-1">{selectedResume ? (
                selectedScore ? (
  <span className={cn("flex items-center gap-1 font-medium min-w-0", isSelectedScoreLow ? "text-red-500" : selectedScore?.tag === "medium" ? "text-amber-600 dark:text-amber-400" : "text-emerald-600 dark:text-emerald-400")}>{isSelectedScoreLow ? <TrendingDown className="h-3 w-3" /> : selectedScore?.tag === "medium" ? <Minus className="h-3 w-3" /> : <TrendingUp className="h-3 w-3" />}
                    <span className="truncate" title={selectedResume.label}>{selectedScore.resume_score.toFixed(0)}% match - {selectedResume.label}
                    </span>
                  </span>
            ...
            <div className="flex gap-2 shrink-0">
              ...
              <Button size="sm" disabled={!selected || applying || loading} onClick={handleApplyClick} className={cn("rounded-xl min-w-[110px]", isSelectedScoreLow ? "bg-amber-500 hover:bg-amber-600 text-white" : "bg-primary hover:bg-primary/90")}>{applying ? <><Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" /> Submitting</> : isSelectedScoreLow ? <><ShieldAlert className="h-3.5 w-3.5 mr-1.5" /> Apply Anyway</> : <><Zap className="h-3.5 w-3.5 mr-1.5 fill-current" /> Apply Now</>}
              </Button>
            </div>
          </div>
        </DialogContent>
        ...
        <DialogContent className="max-w-sm rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm leading-relaxed">
              Your resume scored <span className="font-bold text-red-500">{selectedScore?.resume_score.toFixed(0)}%</span> for <span className="font-medium text-foreground">{jobTitle}</span>. This is below the recommended threshold, so you may be less competitive against other applicants.
            </DialogDescription>
          </DialogHeader>{selectedScore && (
