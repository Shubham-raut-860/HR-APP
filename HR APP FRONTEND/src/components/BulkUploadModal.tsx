/**
 * BulkUploadModal — Chunked batch upload with per-file status tracking
 *
 * ══════════════════════════════════════════════════════════════════
 * BUG FIXES vs previous version
 * ══════════════════════════════════════════════════════════════════
 *
 * [BUG FIX #1 — CRITICAL: "No response from server" for some files]
 ...
 *
 * [BUG FIX #2 — skipped_duplicates not surfaced to the user]
 ...
 *
 * [BUG FIX #3 — BATCH_CONCURRENCY × BATCH_SIZE overloads backend AI semaphore]
 *   BATCH_CONCURRENCY=2, BATCH_SIZE=20 → 40 files in-flight at once.
 ...
 *   Fix: reduced BATCH_SIZE to 10, BATCH_CONCURRENCY stays at 2 → max 20
 *   files in-flight, well within the backend semaphore.
 *
 * [BUG FIX #4 — timeout: 0 can hang forever on network drop]
 ...
 *   Fix: 4-minute timeout per batch (240s). 10 files × ~20s per AI call = 200s
...
/**
 * BUG FIX #3: Reduced from 20 → 10 to stay within backend AI semaphore(10).
 * 500 files ÷ 10 = 50 batches × 2 concurrent = manageable load.
 */
const BATCH_SIZE = 10;

/** 2 concurrent batch chains → max 20 files in-flight at once. */
const BATCH_CONCURRENCY = 2;

/**
 * BUG FIX #4: 4-minute timeout per batch request.
 * 10 files × ~20s worst-case AI parse = 200s + 40s headroom.
 */
...
async function uploadBatch(
  ...
  for (const tf of trackedBatch) {
    ...
    formData.append("file_ids", tf.id);  // new field — backend echoes this back
  }
...
export function BulkUploadModal({ jobs, onUploadComplete, trigger }: BulkUploadModalProps) {
  ...
  // Auto-collapse when >50 files — rendering 500 rows is expensive and unhelpful
  ...
  const addFiles = useCallback((incoming: File[]) => {
    ...
    setTrackedFiles(prev => {
      // Deduplicate by name+size — prevent double-adding the same file
        ...
        <div className="px-6 py-5 space-y-5 flex-1 overflow-y-auto">
          ...
          {total > 0 && (
            <div className="space-y-2">
              ...
              {/* Full file list — only when not summary-mode */}
              {!listCollapsed && (
