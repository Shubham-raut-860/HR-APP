...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (isTechnicalAdmin(user)) {
    navItems.splice(4, 0, { label: "Inspector", path: "/developer/inspector", icon: ShieldCheck });
    navItems.splice(5, 0, { label: "Developer", path: "/developer/a2a", icon: Bot });
  }
            ...
            return (
              ...
              >
                ...
                {!isCollapsed && label === 'Candidates' && untaggedCount > 0 && (
                  <span className="hidden h-[18px] min-w-[18px] px-1 rounded-full bg-destructive text-destructive-foreground text-[10px] font-bold sm:flex items-center justify-center">
