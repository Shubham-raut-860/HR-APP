...
import { ArrowUpDown, ArrowUp, ArrowDown, Calendar, CheckCircle2, X, XCircle, ChevronRight, ChevronLeft, ChevronsLeft, ChevronsRight, User, SlidersHorizontal, Scale } from 'lucide-react';
import { DropdownMenu, DropdownMenuContent, DropdownMenuTrigger } from "@/components/ui/dropdown-menu";
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { toast } from "sonner";
import { draftCandidateEmail, sendCandidateEmail, updateCandidate } from "@/services/candidates";
import { ShortlistCandidateSheet } from "@/components/shortlist/ShortlistCandidateSheet";
import { ShortlistEmailDialog, type ShortlistEmailDraft } from "@/components/shortlist/ShortlistEmailDialog";
...
export const ShortlistTable = memo(function ShortlistTable({ candidates, jobs = [], isLoading, onRefresh }: ShortlistTableProps) {
  ...
  const [emailDraft, setEmailDraft] = useState<ShortlistEmailDraft>({ subject: '', body: '', candidateId: '', candidateName: '', toEmail: '', jobTitle: '' });
  const [draftingEmail, setDraftingEmail] = useState(false);
  const [sendingEmail, setSendingEmail] = useState(false);
  const [updatingTagId, setUpdatingTagId] = useState<string | null>(null);
  const [optimisticTags, setOptimisticTags] = useState<Record<string, Candidate["tag"]>>({});

  const effectiveTag = (c: Candidate): Candidate["tag"] => optimisticTags[c.id] ?? c.tag;
  ...
  const jobTitleById = useMemo(
    () => new Map(jobs.map((j) => [j.id, j.title || j.role || 'Unknown Role'])),
    [jobs]
  );
  const jobTitleForCandidate = (candidate: Candidate) => jobTitleById.get(candidate.job_id) ?? 'Unknown Role';
  const compareCandidates = useMemo(
    () => candidates.filter((candidate) => compareIds.has(candidate.id)).slice(0, 3),
  ...
  const toggleCompare = (candidate: Candidate) => {
    if (!compareIds.has(candidate.id) && compareIds.size >= 3) {
      toast.info('Compare up to 3 candidates at a time.');
      return;
    }
    setCompareIds((prev) => {
      const next = new Set(prev);
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    return [...candidates]
      .filter(c => {
        ...
        // Status filter
        const tag = effectiveTag(c);
        if (selectedStatuses.size > 0 && !selectedStatuses.has(tag)) return false;
      ...
      .sort((a, b) => {
        ...
        if (sortField === 'tag')          return (effectiveTag(a) || '').localeCompare(effectiveTag(b) || '') * mod;
  ...
  const handleDraftEmail = async (c: Candidate, type: 'invite' | 'reject' | 'offer') => {
    ...
    try {
      ...
      setEmailDraft({ subject: data.subject || '', body: data.body || '', candidateId: c.id, candidateName: c.name, toEmail: c.email || 'No email', jobTitle: jobTitleForCandidate(c) });
  ...
  const handleQuickTag = async (c: Candidate, nextTag: Candidate["tag"]) => {
    if (effectiveTag(c) === nextTag) return;
    setUpdatingTagId(c.id);
    setOptimisticTags(prev => ({ ...prev, [c.id]: nextTag }));
    try {
      await updateCandidate(c.id, { tag: nextTag });
      toast.success(`${c.name} marked as ${nextTag}`);
      await onRefresh?.();
    } catch {
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              paginated.map(c => {
                const tag = effectiveTag(c);
                const compared = compareIds.has(c.id);
                return (
                <TableRow
                  key={c.id}
                ...
                >
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <Button
                        size="icon"
                        variant={tag === 'Strong' ? 'default' : 'outline'}
                        className="h-8 w-8 rounded-full"
                        disabled={updatingTagId === c.id}
                        onClick={e => { e.stopPropagation(); void handleQuickTag(c, 'Strong'); }}
                        title="Mark Strong"
                      >
                      ...
                      <Button
                        size="icon"
                        variant={tag === 'Medium' ? 'default' : 'outline'}
                        className="h-8 w-8 rounded-full"
                        disabled={updatingTagId === c.id}
                        onClick={e => { e.stopPropagation(); void handleQuickTag(c, 'Medium'); }}
                        title="Mark Medium"
                      >
                      ...
                      <Button
                        size="icon"
                        variant={tag === 'Reject' ? 'destructive' : 'outline'}
                        className="h-8 w-8 rounded-full"
                        disabled={updatingTagId === c.id}
                        onClick={e => { e.stopPropagation(); void handleQuickTag(c, 'Reject'); }}
                        title="Mark Reject"
                      >
      ...
      <Dialog open={compareOpen} onOpenChange={setCompareOpen}>
        <DialogContent className="max-w-5xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Compare shortlisted profiles side by side before moving to the next step.
            </DialogDescription>
          </DialogHeader>
          {compareCandidates.length < 2 ? (
            <div className="rounded-2xl border border-dashed p-8 text-center text-sm text-muted-foreground">
              Select at least two candidates from the table to compare.
            </div>
          ) : (
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              {compareCandidates.map((candidate) => {
                const skills = (candidate.normalized_skills?.length ? candidate.normalized_skills : candidate.skills) || [];
                const tag = effectiveTag(candidate);
                return (
                  <div key={candidate.id} className="rounded-2xl border bg-card p-4 shadow-sm">
                    <div className="mb-4 flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="truncate font-semibold">{candidate.name || 'Unknown candidate'}</p>
                        <p className="truncate text-xs text-muted-foreground">{candidate.email || 'No email'}</p>
                      </div>
                      <Button
                        size="icon"
                        variant="ghost"
                        ...
                        aria-label={`Remove ${candidate.name || 'candidate'} from comparison`}
                      >
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div className="rounded-xl bg-primary/5 p-3">
                        ...
                        <p className="text-xl font-bold text-primary">{candidate.resume_score}%</p>
                      </div>
                      ...
                      <div className="rounded-xl bg-muted/40 p-3">
                        ...
                        <p className="line-clamp-2 text-sm font-medium">{jobTitleForCandidate(candidate)}</p>
                      </div>
                      <div className="rounded-xl bg-muted/40 p-3">
                        <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Experience</p>
                        <p className="text-sm font-medium">{candidate.experience_years ?? 0} yrs</p>
                      </div>
                    </div>
                    <div className="mt-4">
                      <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Top skills</p>
                      <div className="flex flex-wrap gap-1.5">
                        {skills.slice(0, 6).map((skill) => (
                          <Badge key={skill} variant="secondary" className="rounded-lg text-[10px]">
                            {skill}
                          </Badge>
                        ))}
                        {skills.length === 0 && <span className="text-xs text-muted-foreground">No skills captured</span>}
                      </div>
                    </div>
                    <Button
                      variant="outline"
