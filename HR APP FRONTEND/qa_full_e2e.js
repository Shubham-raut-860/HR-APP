...
async function clickSpecificTopicMode(page) {
  const tabLocator = page.getByRole('tab', { name: /Specific Topic/i }).first();
  const buttonLocator = page.getByRole('button', { name: /Specific Topic/i }).first();

  try {
    await tabLocator.waitFor({ timeout: 8000 });
    await tabLocator.click();
    return;
  } catch {
    // Fallback for legacy/alternate render where mode selector is still a button.
  }

  await buttonLocator.waitFor({ timeout: 12000 });
  await buttonLocator.click();
}

async function signup(page, user, role) {
  const roleParam = role === 'hr' ? 'hr' : 'candidate';
  await page.goto(`${FRONTEND}/signup?role=${encodeURIComponent(roleParam)}`, { waitUntil: 'networkidle' });
...
(async () => {
  ...
  // Candidate flow
  {
    ...
    try {
      ...
      await clickSpecificTopicMode(page);
