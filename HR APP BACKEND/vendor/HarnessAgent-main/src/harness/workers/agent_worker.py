    ...
    def _build_agent(agent_type: str, redis_client: Any = None) -> Any:
        ...
        try:
            mlflow_tracer = MLflowAgentTracer(
                tracking_uri=config_dict.get("mlflow_tracking_uri", cfg.mlflow_tracking_uri),
            ...
            logger.debug("MLflow tracer init skipped for agent_type=%s: %s", agent_type, exc)
...
def _build_registered_agent(agent_type: str, cfg: Any, redis_client: Any) -> Any:
    """Build an agent instance for API-side direct runner execution."""
    from harness.core.cost_tracker import CostTracker
    from harness.llm.factory import build_router
    from harness.observability.mlflow_tracer import MLflowAgentTracer
    from harness.observability.trace_recorder import TraceRecorder
    ...
    common_kwargs = {
        ...
        "checkpoint_manager": _NoopCheckpointManager(),
        "message_bus": None,
        "trace_recorder": TraceRecorder.create(redis_url=cfg.redis_url),
    }
...
def build_agent_factory(cfg: Any) -> Any:
    ...
    def _factory(agent_type: str) -> Any:
        return _build_registered_agent(agent_type, cfg=cfg, redis_client=_redis)
