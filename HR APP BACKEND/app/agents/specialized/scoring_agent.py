...
class ScoringAgent(BaseAgent):
    ...
    async def run(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        ai_timeout_s = max(
            3.0,
            min(
                float(state.get("timeout_s", 45.0)),
        ...
        async def _run_specialist(name: str, fn: Any, *args: Any, fallback: float = 0.0) -> float:
            ...
            try:
                ...
                logger.warning("ScoringAgent specialist %s failed, using fallback: %s", name, exc)
        ...
        # Run deterministic specialists in parallel for true multi-agent fan-out.
        def _edu_score() -> float:
        ...
        rule_skill, rule_exp, rule_proj, edu_pct, loc_pct = await asyncio.gather(
            _run_specialist(
                "skill_match_specialist",
                scoring_service.skill_match_score,
                normalized_skills,
                job.must_have_skills,
                job.good_to_have_skills,
            ),
            _run_specialist(
                "experience_match_specialist",
                scoring_service.experience_match_score,
                exp_years,
                job.experience_min,
                job.experience_max,
                skill_yrs,
            ...
            _run_specialist(
                "project_relevance_specialist",
                scoring_service.project_relevance_score,
                projects,
                job.must_have_skills,
                job.good_to_have_skills,
                exp_years,
            ),
            _run_specialist("education_match_specialist", _edu_score),
            _run_specialist(
                "location_match_specialist",
                scoring_service.location_match_score,
                location,
                job.location,
            ),
        )
            ...
            async def _pair_similarity(left: str, right: str) -> float | None:
                ...
                try:
                    left_vec, right_vec = await asyncio.wait_for(
                        ...
                        timeout=max(3.0, min(ai_timeout_s, 8.0)),
                    )
                    try:
        ...
        score_result = {
            ...
            "orchestration": {
                ...
                "total_latency_ms": round((time.perf_counter() - run_started) * 1000.0, 2),
            },
        }
            ...
            "_agent_trace": [
                {
                    "agent": self.name,
                    "mode": "parallel_specialists",
                    "specialist_count": len(specialist_traces),
                    "latency_ms": round((time.perf_counter() - run_started) * 1000.0, 2),
                }
            ],
        }
