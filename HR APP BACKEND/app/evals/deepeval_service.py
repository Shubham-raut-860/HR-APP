"""
DeepEval evaluation service for Jobora.

Merges DeepEval (primary eval engine) with MLflow/token monitoring observability.
It provides a unified evaluation layer. Every LLM operation in the platform — resume parsing,
resume scoring, and JD generation — has a dedicated set of metrics here.

Architecture:
  ┌────────────┐    results    ┌─────────────────┐   traces  ┌──────────┐
  │  Jobora    │ ───────────► │  DeepEvalService │ ────────► │ Langfuse │
