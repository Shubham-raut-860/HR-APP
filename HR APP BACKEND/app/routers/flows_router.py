...
async def _persist_flow_run_status(
    run_id: str,
    *,
    status: str,
    ...
    async with AsyncSessionLocal() as s:
        ...
        if details is not None:
            row.details = details
            row.processed = 1 if status == "completed" else 0
            row.failed = 1 if status in {"failed", "cancelled", "rejected"} else 0
...
async def _run_batch_score_subprocess(run_id: str, cmd: list[str], cwd: str, user_email: str) -> None:
    async with _FLOW_DISPATCH_SEMAPHORE:
        ...
        try:
            ...
            await _persist_flow_run_status(
                run_id,
                status="running",
                details=dict(_FLOW_STATUS.get(run_id, {})),
            )
            ...
            await _persist_flow_run_status(
                run_id,
                status=_FLOW_STATUS[run_id]["status"],
                details=dict(_FLOW_STATUS.get(run_id, {})),
                error_summary=None if rc == 0 else {"return_code": rc, "stderr_tail": stderr_tail},
            )
            logger.info(
...
async def dispatch_batch_score(
    ...
    async with AsyncSessionLocal() as s:
        s.add(
            BulkUploadJob(
                ...
                processed=0,
                failed=0,
                details=dict(_FLOW_STATUS[run_id]),
            )
        )
        await s.commit()
    ...
    if _FLOW_QUEUE.full():
        ...
        await _persist_flow_run_status(
            ...
            status="rejected",
            details=dict(_FLOW_STATUS.get(run_id, {})),
            error_summary={"error": "Flow queue is full"},
        )
        raise HTTPException(
...
async def get_batch_score_status(
    ...
    if not status_row:
        async with AsyncSessionLocal() as s:
            row = await s.get(BulkUploadJob, run_id)
            if row and isinstance(row.details, dict) and row.details.get("flow_name") == "BatchScoringFlow":
                status_row = {
                    "run_id": row.id,
                    "status": row.status,
                    **dict(row.details or {}),
