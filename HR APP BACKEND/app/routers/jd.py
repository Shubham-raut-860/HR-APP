    ...
    *,
    ...
    try:
        result = await harness_agent_client.run_agent(
            ...
            timeout_s=min(30.0, max(15.0, float(settings.AI_REQUEST_TIMEOUT_SECONDS) * 2.0)),
        )
        ...
        try:
            return await asyncio.wait_for(
                ...
                timeout=min(30.0, max(15.0, float(settings.AI_REQUEST_TIMEOUT_SECONDS) * 2.0)),
            )
        except Exception as direct_exc:
            logger.exception(
...
async def generate_jd(
    ...
    return _normalize_generated_jd(
