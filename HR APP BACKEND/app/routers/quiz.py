...
from app.models import User, Quiz, Question, QuizAttempt, Candidate, JobDescription, QuizStatus, CandidateTag, NotificationType, UserRole, BulkUploadJob
...
from app.services.adk_promotion_service import try_promoted_quiz_generation
...
_QUIZ_GENERATE_TIMEOUT_S = 14.0
_QUIZ_FALLBACK_TIMEOUT_S = 16.0
_QUIZ_PARSE_TIMEOUT_S = 20.0
_QUIZ_EMAIL_NOTIFY_TIMEOUT_S = 8.0
_QUIZ_CODE_EVAL_TIMEOUT_S = 25.0


def _schedule_quiz_generation_shadow(
    *,
    jd: JobDescription,
...
def _schedule_quiz_validation_shadow(
    *,
    jd: JobDescription,
    questions_data: list[dict[str, Any]],
    user: User,
    ...
    schedule_adk_shadow_observation(
        ...
        production_output={
            "accepted_by_persistence_validation": True,
            "question_count": len(questions_data),
        },
        actor_id=str(user.id),
        entity_id=str(jd.id),
...
async def _update_quiz_run_status(
    run_id: str,
    *,
    status: str,
...
async def _run_quiz_generation_with_fallback(
    *,
    ...
    promoted = await try_promoted_quiz_generation(
        jd_text=jd_text,
        skills=skills,
        easy=easy,
        medium=medium,
    ...
    if promoted is not None and isinstance(promoted.output, list):
...
async def _run_quiz_document_parse_with_fallback(doc_text: str, request: Request | None = None) -> list[dict]:
    try:
        ...
        try:
            ...
            # Keep async quiz-from-file jobs deterministic even when both
            # harness and direct AI parser are unavailable.
            logger.warning(
                "Direct quiz document parser failed; downstream fallback set will be used: %s",
                direct_exc,
            )
            return []
...
def _build_magic_link(raw_token: str) -> str:
    ...
    return f"{frontend_url}/take-quiz?token={raw_token}"
...
async def generate_quiz(
    ...
    if settings.DATABASE_URL.startswith("sqlite"):
        ...
        _schedule_quiz_generation_shadow(jd=jd, questions_data=questions_data, user=user)
        _schedule_quiz_validation_shadow(jd=jd, questions_data=questions_data, user=user)
        return QuizOut(
...
async def _run_create_quiz_from_file_job(
    *,
    run_id: str,
    requested_by: str,
    job_id: str,
    ...
    async def _execute() -> None:
        async with AsyncSessionLocal() as db:
            try:
                ...
                if not jd:
                    raise ValueError("Job not found for queued quiz generation")

                doc_text = await file_service.extract_text_from_bytes(filename or "quiz.pdf", content_bytes)
                if not doc_text.strip():
                    raise ValueError("Could not extract any text from uploaded file")

                questions_data = await _run_quiz_document_parse_with_fallback(doc_text, None)
                if not questions_data:
                    questions_data = _fallback_quiz_questions(
                        skills=(jd.must_have_skills or []) + (jd.good_to_have_skills or []),
                        easy=settings.QUIZ_EASY_COUNT,
                ...
                if not questions_data:
                    raise ValueError("No valid MCQ questions found in uploaded document")

                fname = os.path.splitext(filename or "Quiz")[0].replace("_", " ").replace("-", " ").title()
                quiz = Quiz(
                    job_id=jd.id,
                ...
                await log_action(
                    ...
                    "quiz",
                    quiz.id,
                    details={"filename": filename, "question_count": len(questions_data), "queued_run_id": run_id},
                )
                await db.commit()

                await _update_quiz_run_status(
                    run_id,
                    status="completed",
                    processed=1,
                    failed=0,
                    details={
                        "run_type": "quiz_from_file",
                        "job_id": job_id,
                        "filename": filename,
                        "question_count": len(questions_data),
                        "quiz_id": quiz.id,
                        ...
                        "duration_ms": round((asyncio.get_event_loop().time() - started) * 1000.0, 2),
                    },
                )
            except Exception as exc:
                logger.exception("Queued quiz from-file failed run_id=%s", run_id)
                await db.rollback()
                await _update_quiz_run_status(
                    run_id,
                    status="failed",
    ...
    try:
        ...
        logger.warning("Queued quiz from-file timed out run_id=%s", run_id)
        await _update_quiz_run_status(
            run_id,
            status="failed",
            processed=0,
...
@router.post("/from-file-async", status_code=202)
async def create_quiz_from_file_async(
    request: Request,
    ...
    if not jd:
        raise HTTPException(status_code=404, detail="Job not found")
    if user.role != UserRole.admin and jd.created_by != user.id:
        raise HTTPException(status_code=403, detail="You do not have permission to add a quiz to this job")
    ...
    db.add(
        BulkUploadJob(
            ...
            details={
                "run_type": "quiz_from_file",
                "job_id": jd.id,
                "filename": file.filename or "quiz.pdf",
                "queued_at": _utc_now_iso(),
                "status_url": f"/quiz/from-file-runs/{run_id}",
            },
        )
    )
    await db.commit()
    ...
    background_tasks.add_task(
        _run_create_quiz_from_file_job,
        run_id=run_id,
    ...
    return {
        "run_id": run_id,
        "status": "queued",
        "poll_url": f"/quiz/from-file-runs/{run_id}",
    }


@router.get("/from-file-runs/{run_id}")
async def create_quiz_from_file_run_status(
    run_id: str,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(require_hr),
):
    row = await db.get(BulkUploadJob, run_id)
    if not row or (row.details or {}).get("run_type") != "quiz_from_file":
        raise HTTPException(status_code=404, detail="Run not found")
    if user.role != UserRole.admin and row.created_by and row.created_by != user.id:
...
async def _dispatch_quiz_links_background(
    ...
    if not links_to_send:
        return {"success_count": 0, "failed_count": 0, "failed_candidate_ids": []}

    from app.services.email_service import send_email_with_retry, _redact_email
    ...
    return {
        "success_count": len(successful_candidate_ids),
...
async def send_quiz_links(
    ...
    for cid in candidate_ids:
        ...
        cand_email = candidate.email or "[email-redacted]"
    ...
    await db.commit()
    email_delivery = await _dispatch_quiz_links_background(new_links, quiz.title, quiz.id)
    status_code = 207 if email_delivery["failed_count"] else 200

    return JSONResponse(status_code=status_code, content={
        "message": f"Quiz link dispatch attempted for {len(new_links)} candidates",
        "links": links,
        "created_count": created,
        "rotated_count": rotated,
...
async def submit_quiz(
    ...
    if attempt.status == QuizStatus.submitted and idem_key:
        ...
        return QuizResultOut(
            ...
            difficulty_breakdown=attempt.difficulty_breakdown if isinstance(attempt.difficulty_breakdown, dict) else {},
            passed=attempt.candidate.passed if attempt.candidate else None,
        )
