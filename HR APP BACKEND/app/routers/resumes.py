...
async def _compute_resume_data_from_bytes(
    ...
    #  Step 2: Rule-based scores (always; AI fallback) 
    ...
    critical_missing_count = sum(
        ...
        if not scoring_service.semantic_skill_match(skill, parsed.get("normalized_skills", []))
    )
    rule_exp_pct = scoring_service.experience_match_score(
        exp_years, job.experience_min, job.experience_max,
        skill_years, job.must_have_skills or [],
    )
...
async def bulk_archive_candidates(
    ...
    try:
        if user.role != UserRole.admin:
            owned_job_ids = (await db.execute(
                select(JobDescription.id).where(JobDescription.created_by == user.id)
            )).scalars().all()

            unowned = (await db.execute(
            ...
            # BUG 15 FIX: pool candidates (job_id IS NULL) were skipped by the
            # isnot(None) filter above, letting any HR user archive any pool
            # candidate. Now restrict pool mutations to the uploader only.
            unowned_pool = (await db.execute(
                select(Candidate.id).where(
                    Candidate.id.in_(body.candidate_ids),
                    Candidate.job_id.is_(None),
                    Candidate.user_id != user.id,
...
async def bulk_restore_candidates(
    ...
    try:
        # BUG 3 FIX: add the same ownership guard that bulk-archive has.
        # Without this any HR user could restore any other user's candidates.
        if user.role != UserRole.admin:
            owned_job_ids = (await db.execute(
                select(JobDescription.id).where(JobDescription.created_by == user.id)
            )).scalars().all()
            ...
            # BUG 15 FIX: pool candidates bypass the isnot(None) filter above.
            # Restrict pool restore to the uploader only.
            unowned_pool = (await db.execute(
                select(Candidate.id).where(
                    Candidate.id.in_(body.candidate_ids),
                    Candidate.job_id.is_(None),
...
async def bulk_delete_candidates(
    ...
    try:
        if user.role != UserRole.admin:
            ...
            # BUG 1 FIX: pool candidates (job_id IS NULL) were invisible to the
            # isnot(None) filter above, so any HR user could hard-delete any other
            # HR user's pool candidates. Same pattern already applied to
            # bulk_archive and bulk_restore  was simply missed here.
            unowned_pool = (await db.execute(
                select(Candidate.id).where(
                    Candidate.id.in_(body.candidate_ids),
                    Candidate.job_id.is_(None),
