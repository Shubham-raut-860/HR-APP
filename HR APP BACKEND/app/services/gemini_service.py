...
def _is_unsupported_sampling_error(exc: Exception) -> bool:
    ...
    return (
        ...
        and ("temperature" in text or "top_p" in text or "seed" in text)
    )
...
async def _generate_json(
    ...
    for attempt in range(max_retries):
        try:
            ...
            if task_name in _DETERMINISTIC_SCORING_TASKS and allow_sampling_params:
            ...
            # Some Azure model deployments reject explicit temperature/top_p/seed
            # overrides. Retry once without sampling params so the call still succeeds.
            if (
                task_name in _DETERMINISTIC_SCORING_TASKS
                and allow_sampling_params
                and _is_unsupported_sampling_error(exc)
            ):
                logger.warning(
                    "[Azure OpenAI] task=%s deployment=%s rejected explicit sampling params; retrying without overrides",
                    task_name,
                    deployment,
                )
