...
def _register_prompts() -> None:
    """
    Log Jobora system prompts to MLflow so they appear in the Prompts section.
    ...
    try:
        ...
        with mlflow.start_run(
            ...
            tags={"type": "prompt_registry", "source": "jobora"},
        ):
...
def _register_azure_model() -> None:
    ...
    try:
        ...
        with mlflow.start_run(
            run_name="jobora_azure_openai_registration",
            experiment_id=_experiment_id,
            tags={"type": "model_registration", "source": "jobora"},
        ) as run:
            mlflow.log_params({
            ...
            model_info = mlflow.openai.log_model(
                ...
                artifact_path="jobora_chat_model",
                messages=[
                    {"role": "system",  "content": "You are an HR software suite."},
...
def tag_trace_with_session(
    ...
    try:
        ...
        mlflow.set_tags({
            ...
            "jobora.user_id":    str(user_id) if user_id else "anonymous",
            "jobora.request_id": str(request_id or session_id),
            "jobora.run_id": str(run_id or ""),
            "jobora.app_env": str(app_env or ""),
        })
    except Exception as exc:
...
async def mlflow_track_llm(
    ...
    try:
        ...
        run_tags = {
            ...
            **({"jobora.user_id": str(user_id)} if user_id else {}),
            **(tags or {}),
        }
...
def run_mlflow_evaluation(
    ...
    """
    Run mlflow.evaluate() with our custom Jobora judge functions.
    ...
    try:
        ...
        dataset = mlflow.data.from_pandas(
            df,
            name=f"jobora_{operation}_dataset",
    ...
    try:
        ...
        metrics.append(
            mlflow.metrics.make_metric(
                ...
                name="jobora_completeness",
                long_name=f"Jobora Output Completeness ({operation})",
            )
        )
        metrics.append(
            mlflow.metrics.make_metric(
                ...
                greater_is_better=True,
                name="jobora_relevance",
                long_name=f"Jobora Output Relevance ({operation})",
            )
        )
        ...
        if operation == "resume_scoring":
            ...
            metrics.append(
                mlflow.metrics.make_metric(
                    ...
                    greater_is_better=True,
                    name="jobora_score_range_validity",
                    long_name="Jobora Score Range Validity (0-100)",
                )
            )
    ...
    def start_run(self, run_name: str = ""):
        ...
        try:
            ...
            run = mlflow.start_run(
                run_name=run_name or "jobora_llm_call",
                experiment_id=_experiment_id,
            )
