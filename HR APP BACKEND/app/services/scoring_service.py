...
def _ai_confidence_base_weight(confidence: str | None) -> float:
    """
    Convert LLM confidence labels into a bounded trust prior.

    Deliberately conservative: even "high" confidence keeps rule-based evidence
    in the loop to avoid overpowered single-call scoring.
    """
...
def _ai_rule_trust_weight(
    *,
    ...
    jd_signal_strength: float | None,
    critical_missing_count: int,
) -> float:
    """
    Compute a bounded trust weight for AI component scores.

    Higher disagreement between AI and deterministic rubric lowers trust.
    This prevents both overpowered and underpowered outcomes from noisy LLM
    responses while still benefiting from semantic understanding.
    """
    base = _ai_confidence_base_weight(ai_confidence)
    disagreement = (
        abs(ai_skill - rule_skill)
        + abs(ai_exp - rule_exp)
        + abs(ai_proj - rule_proj)
    ) / 3.0
    # 0.0 (perfect agreement) .. 1.0 (very large disagreement)
    disagreement_norm = _clamp01(disagreement / 55.0)
    agreement_factor = 1.0 - disagreement_norm
    trust = base * (0.55 + (0.45 * agreement_factor))
    ...
    if signal is not None:
        # Low-signal JDs are noisy inputs; reduce AI dominance.
        if signal < 0.35:
            trust *= 0.86
        elif signal < 0.50:
            trust *= 0.92

    # If must-have gaps exist and AI heavily overstates skill fit, rely more on rule evidence.
    if int(critical_missing_count or 0) > 0 and (ai_skill - rule_skill) > 15.0:
        trust *= 0.85

    return round(max(0.35, min(0.85, trust)), 4)
...
def compute_resume_score_with_ai_override(
    ...
    ai_path_eligible = (
        ...
        and all(isinstance(ai_scores.get(k), (int, float)) for k in ("skill_score", "experience_score", "project_score"))
    )

    if ai_path_eligible:
        skill_pct = _clamp_pct(float(ai_scores["skill_score"]))
        exp_pct = _clamp_pct(float(ai_scores["experience_score"]))
        proj_pct = _clamp_pct(float(ai_scores["project_score"]))
        ...
        trust_weight = _ai_rule_trust_weight(
            ai_skill=skill_pct,
            ai_exp=exp_pct,
            ai_proj=proj_pct,
            rule_skill=rule_skill_pct,
            rule_exp=rule_exp_pct,
            rule_proj=rule_proj_pct,
            ai_confidence=ai_confidence or str(ai_scores.get("confidence", "")),
            jd_signal_strength=jd_signal_strength,
            critical_missing_count=critical_missing_count,
        )
        rule_weight = 1.0 - trust_weight

        # Precision calibration: blend AI semantics with deterministic evidence.
        # This reduces both over-scoring and under-scoring outliers.
        skill_pct = (skill_pct * trust_weight) + (rule_skill_pct * rule_weight)
        exp_pct = (exp_pct * trust_weight) + (rule_exp_pct * rule_weight)
        proj_pct = (proj_pct * trust_weight) + (rule_proj_pct * rule_weight)

        # Preserve hard domain mismatch guardrails even after blending.
        if domain_fit == "different":
            skill_pct = min(skill_pct, 38.0)
            exp_pct = min(exp_pct, 35.0)
        elif domain_fit == "adjacent":
