...
class HRMultiAgentRuntime:
    ...
    def __init__(self) -> None:
        self._agents = {
            ...
            "recruiter_assistant_agent": RecruiterAssistantAgent(),
            "candidate_coach_agent": CandidateCoachAgent(),
        ...
        self._semaphores = {
            name: asyncio.Semaphore(heavy_limit if name in self._heavy_agents else default_limit)
            for name in self._agents
        }

    def _agent_timeout(self, state: dict[str, Any]) -> float:
        raw = state.get("timeout_s")
        cap = max(5.0, float(getattr(settings, "AGENT_MAX_TIMEOUT_SECONDS", 90.0)))
        try:
            requested = float(raw) if raw is not None else 45.0
        except (TypeError, ValueError):
            requested = 45.0
        return max(5.0, min(requested, cap))
    ...
    async def run_agent(self, agent_name: str, state: dict[str, Any]) -> dict[str, Any]:
        ...
        timeout_s = self._agent_timeout(state)
        semaphore = self._semaphores.get(agent_name)
        try:
            async def _run() -> dict[str, Any]:
                if semaphore is None:
                    return await agent(dict(state))
                async with semaphore:
                    return await agent(dict(state))

            updates = await asyncio.wait_for(_run(), timeout=timeout_s)
        except AgentError as exc:
            raise HRMultiAgentRuntimeError(agent_name, str(exc)) from exc
        except asyncio.TimeoutError as exc:
            raise HRMultiAgentRuntimeError(agent_name, f"agent timeout after {timeout_s:.1f}s") from exc
    ...
    async def run_resume_pipeline(
        ...
        *,
        ...
        state: dict[str, Any] = {
            ...
            "skip_ai_scoring": bool(skip_ai_scoring),
        }
        ...
        async def _pipeline() -> dict[str, Any]:
            ...
            dedup_task = self.run_agent("deduplication_agent", state)
            scoring_task = self.run_agent("scoring_agent", state)
            dedup_updates, scoring_updates = await asyncio.gather(dedup_task, scoring_task)
            self._merge_updates(state, dedup_updates)
            self._merge_updates(state, scoring_updates)
            return state
    ...
    async def validate_quiz(
        ...
        *,
        ...
        report = updates.get("quiz_validation")
        if not isinstance(report, dict):
            raise HRMultiAgentRuntimeError("quiz_agent", "missing quiz_validation")
        return report
    ...
    async def run_recruiter_copilot(
        ...
        *,
        ...
        result = updates.get("recruiter_copilot")
        if not isinstance(result, dict):
            raise HRMultiAgentRuntimeError("recruiter_assistant_agent", "missing recruiter_copilot")
        return result
    ...
    async def run_candidate_coach(
        ...
        *,
        ...
        result = updates.get("candidate_coach")
        if not isinstance(result, dict):
            raise HRMultiAgentRuntimeError("candidate_coach_agent", "missing candidate_coach")
        return result
