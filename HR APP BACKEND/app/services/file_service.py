...
def _configure_tesseract_path() -> None:
    ...
    for path in candidates:
        if os.path.isfile(path):
            ...
            logger.info("Tesseract found at %s - configured automatically.", path)
            return
...
# Known signatures for basic content-extension integrity checks.
...
def validate_file_magic(content: bytes, claimed_ext: str) -> None:
    """Reject files whose bytes do not match their declared extension."""
    ext = (claimed_ext or "").lower()
    if ext == ".txt":
        return
...
def _looks_like_internal_ciphertext(content: bytes) -> bool:
    ...
    """
    return encryption_service.looks_like_internal_ciphertext(content)
...
def _run_tesseract(img: Image.Image) -> str:
    ...
    if not _TESSERACT_AVAILABLE:
        logger.warning("Tesseract not available; OCR skipped for current image input.")
        return ""
...
def _text_looks_valid(text: str) -> bool:
    ...
    # Keep unicode-friendly printable checks so non-English resumes are not
    # rejected solely for containing non-ASCII characters.
    printable_chars = sum(1 for c in text if c.isprintable() or c.isspace())
    printable_ratio = printable_chars / len(text)
    if printable_ratio < 0.80:
        return False

    # Too much non-ASCII noise is a common signal of broken text extraction.
    non_ascii_chars = sum(1 for c in text if ord(c) > 127)
    if (non_ascii_chars / len(text)) > 0.30:
        return False
    ...
    anchors = ("experience", "education", "skills", "summary", "name", "email")
...
def extract_text_from_pdf(content: bytes) -> str:
    ...
    # Mixed PDF: always OCR weak pages by default, even when most of the PDF is
    # digital text. This catches common real resumes where page 1 is selectable
    # text but page 2 is a scanned certificate, transcript, or image-only annex.
    weak_ratio = len(weak_pages) / max(1, len(parts))
    if (
        not settings.OCR_MIXED_PDF_WEAK_PAGES_ENABLED
        and weak_ratio <= 0.20
        and len(digital_text) >= (_PDF_TEXT_THRESHOLD // 2)
    ):
        return digital_text
...
async def extract_text(file: UploadFile) -> tuple[str, bytes]:
    ...
    Returns:
        (extracted_text: str, normalized_plaintext_bytes: bytes)
    """
    max_bytes = int(settings.MAX_FILE_SIZE_MB * 1024 * 1024)
    headers = getattr(file, "headers", None) or {}
    if not isinstance(headers, dict):
        try:
            ...
            headers = {}
    try:
        hinted_size = int(headers.get("content-length") or 0)
    except (TypeError, ValueError):
        hinted_size = 0
    if hinted_size and hinted_size > max_bytes:
        raise HTTPException(
            status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE,
            detail=f"File exceeds the {settings.MAX_FILE_SIZE_MB} MB size limit",
        )
    ...
    if not callable(reader):
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="Uploaded file object is missing a readable stream.",
        )

    raw_content = await reader()
    ...
    if not isinstance(raw_content, bytes):
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="Uploaded file content must be bytes.",
        )

    return await extract_text_normalised_from_bytes(filename, raw_content)


async def extract_text_normalised_from_bytes(filename: str, raw_content: bytes) -> tuple[str, bytes]:
    """
    Extract text and return the canonical bytes used for extraction.
    If the uploaded payload is an internal encrypted artifact, bytes are
    decrypted before hash/save/parse so downstream flows stay consistent.
    """
    ext = _validate_extension(filename)
    _validate_size(raw_content)
    normalized_content = _maybe_decrypt_uploaded_content(filename, ext, raw_content)
    validate_file_magic(normalized_content, ext)
    text = await _dispatch_extraction(ext, normalized_content)
    ...
    if not text.strip():
        raise HTTPException(
            ...
            detail=(
                ...
                "and the page is not rotated more than 45 degrees."
            ),
        )
...
async def extract_text_from_bytes(filename: str, content: bytes) -> str:
    ...
    """
    text, _normalized = await extract_text_normalised_from_bytes(filename, content)
    return text
    ...
    def _prepare_and_write(upload_dir: str, sname: str, data: bytes) -> str:
        ...
        while True:
            try:
                ...
                raise RuntimeError(
                    f"Upload storage is not writable for '{dest}'"
                ) from exc
        return str(dest)
