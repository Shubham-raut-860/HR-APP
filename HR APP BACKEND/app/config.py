...
class Settings(BaseSettings):
    ...
    # Candidate KYC documents are stored in a separate DB to isolate sensitive
    # identity document metadata from the primary hiring dataset.
    KYC_DATABASE_URL: str = "sqlite+aiosqlite:///./candidate_kyc.db"
    ...
    SMTP_USERNAME: str = ""
    SMTP_PASSWORD: str = ""
    SMTP_FROM_EMAIL: str = "[email-redacted]"

    SECRET_KEY: str = ""
    HR_REGISTRATION_MODE: str = "open"  # open | invite
    HR_INVITE_CODES: str = ""
    ALGORITHM: str = "HS256"
    ...
    # - True (default): route execution through Harness first, then fallback
    #   to runtime if Harness is unavailable for resiliency.
    # - False: keep Harness mounted for inspection/tracing/evals only.
    HARNESS_EXECUTION_ENABLED: bool = True
    HARNESS_TRACE_RECORDER_ENABLED: bool = True
    # Unified multi-agent execution controls (latency + stability guards).
    AGENT_MAX_TIMEOUT_SECONDS: float = 90.0
    AGENT_RUNTIME_CONCURRENCY_DEFAULT: int = 12
    AGENT_RUNTIME_CONCURRENCY_HEAVY: int = 6
    AGENT_SINGLEFLIGHT_ENABLED: bool = True
    AGENT_RESULT_CACHE_TTL_SECONDS: float = 45.0
    AGENT_RESULT_CACHE_MAX_ITEMS: int = 1024
    AGENT_PAYLOAD_MAX_CHARS: int = 800000
    TECHNICAL_ADMIN_LOGIN_ENABLED: bool = True
    TECHNICAL_ADMIN_EMAIL: str = "[email-redacted]"
    A2A_SERVICE_TOKENS: str = ""
    A2A_PERSISTENCE_ENABLED: bool = False
    A2A_PERSISTENCE_PATH: str = "a2a_state/a2a_tasks.json"
    ADK_SHADOW_MODE_ENABLED: bool = False
    ADK_SHADOW_EXECUTION_MODE: str = "record_only"  # record_only | runtime_compare
    ADK_SHADOW_WORKFLOW_ALLOWLIST: str = "jd_generation,quiz_generation,quiz_validation,resume_screening"
    ADK_SHADOW_SAMPLE_RATE: float = 1.0
    ADK_SHADOW_TIMEOUT_SECONDS: float = 20.0
    ADK_SHADOW_MAX_EVENTS: int = 500
    ADK_SHADOW_MAX_PAYLOAD_CHARS: int = 20000
    ADK_PROMOTION_ENABLED: bool = False
    ADK_PROMOTION_WORKFLOW_ALLOWLIST: str = ""
    ADK_PROMOTION_TIMEOUT_SECONDS: float = 15.0
    ADK_PROMOTION_FALLBACK_TO_LEGACY: bool = True
    ADK_PROMOTION_MIN_QUIZ_QUALITY_SCORE: float = 70.0
    ...
    REDIS_STARTUP_CONNECT_TIMEOUT_SECONDS: float = 1.5
    REDIS_STARTUP_MAX_ATTEMPTS: int = 4
    REDIS_STARTUP_BACKOFF_SECONDS: float = 0.75
    LIMITER_STRICT_MODE: bool = False

    # ─── Files ────────────────────────────────────────────────────────────────
    UPLOAD_DIR: str = "uploads"
    MAX_FILE_SIZE_MB: int = 20
    OCR_MIXED_PDF_WEAK_PAGES_ENABLED: bool = True
    ...
    KYC_ALLOWED_EXTENSIONS: str = ".pdf,.png,.jpg,.jpeg,.webp"
    KYC_MAGIC_LINK_EXPIRE_MINUTES: int = 45
    KYC_DEFAULT_RETENTION_DAYS: int = 60
    KYC_RETENTION_CLEANUP_INTERVAL_SECONDS: int = 3600
    KYC_RECRUITER_RAW_ACCESS_ENABLED: bool = False
    ...
    @property
    def is_preproduction(self) -> bool:
        return (self.APP_ENV or "").strip().lower() in {"preproduction", "staging", "stable"}

    @property
    def is_release_env(self) -> bool:
        return self.is_production or self.is_preproduction
    ...
    @classmethod
    def validate_frontend_url(cls, value: str | None) -> str:
    ...
    @field_validator("HR_REGISTRATION_MODE", mode="before")
    @classmethod
    def normalize_hr_registration_mode(cls, value: str | None) -> str:
        normalized = str(value or "open").strip().lower()
        if normalized not in {"open", "invite"}:
            raise ValueError("HR_REGISTRATION_MODE must be either 'open' or 'invite'.")
        return normalized
    ...
    def _production_guard(self) -> "Settings":
        ...
        if failures:
            ...
            message = (
                f"Release configuration guard failed for APP_ENV={self.APP_ENV}:\n"
                f"{details}\n"
    ...
    def validate_production_secrets(self) -> "Settings":
        if self.is_release_env:
            if len((self.SECRET_KEY or "").strip()) < 32:
                raise ValueError("SECRET_KEY must be at least 32 characters long in release environments.")
            if not (self.ENCRYPTION_KEY or "").strip():
                raise ValueError("ENCRYPTION_KEY must be set in release environments.")
            if not (self.AZURE_OPENAI_API_KEY or "").strip():
                raise ValueError("AZURE_OPENAI_API_KEY must be set in release environments.")
            if not (self.AZURE_OPENAI_ENDPOINT or "").strip():
                raise ValueError("AZURE_OPENAI_ENDPOINT must be set in release environments.")
            if self.REDIS_REQUIRED_IN_PRODUCTION and not (self.REDIS_URL or "").strip():
                raise ValueError(
                    "REDIS_URL must be set in release environments when REDIS_REQUIRED_IN_PRODUCTION=true."
                )
            if self.HR_REGISTRATION_MODE != "invite":
                raise ValueError(
                    "HR_REGISTRATION_MODE must be 'invite' in release environments."
                )
        ...
        if self.is_release_env:
            if self.REDIS_REQUIRED_IN_PRODUCTION and not self.LIMITER_STRICT_MODE:
                # Enforce fail-closed limiter behavior for production safety.
                self.LIMITER_STRICT_MODE = True
            if "*" in self.cors_origins_list:
                raise ValueError("CORS_ORIGINS cannot contain wildcard '*' in release environments.")
            unsafe = [
                origin
                for origin in self.cors_origins_list
            ...
            if unsafe:
                raise ValueError(
                    f"CORS_ORIGINS contains localhost entries in release environments: {unsafe}"
                )
            if not self.SMTP_USERNAME or not self.SMTP_PASSWORD:
                warnings.warn(
                    "SMTP_USERNAME / SMTP_PASSWORD are empty in release environments. "
                    "Password-reset and notification emails will silently fail.",
                    stacklevel=2,
                )

        if self.hr_registration_requires_invite and not self.hr_invite_codes_list:
            message = (
                "HR registration invite mode is enabled but HR_INVITE_CODES is empty. "
                "Provide one or more codes via env/secret manager."
            )
            if self.is_release_env:
                raise ValueError(message)
    ...
    _cached_kyc_extensions: list[str] | None = None
    ...
    @property
    def kyc_allowed_extensions_list(self) -> List[str]:
        if self._cached_kyc_extensions is None:
            configured = {e.strip().lower() for e in self.KYC_ALLOWED_EXTENSIONS.split(",") if e.strip()}
            required = {".pdf", ".png", ".jpg", ".jpeg", ".webp"}
            self._cached_kyc_extensions = sorted(configured | required)
        return self._cached_kyc_extensions
    ...
    @property
    def hr_invite_codes_list(self) -> List[str]:
        return [code.strip() for code in (self.HR_INVITE_CODES or "").split(",") if code.strip()]

    @property
    def hr_registration_requires_invite(self) -> bool:
        return self.HR_REGISTRATION_MODE == "invite"
