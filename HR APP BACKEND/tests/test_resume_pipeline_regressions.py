...
def test_job_upload_hash_uses_decrypted_bytes(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    user = SimpleNamespace(id="hr-1", email="[email-redacted]", role=UserRole.hr)
    ...
    with pytest.raises(RuntimeError, match="could not be saved"):
        asyncio.run(
            resumes._compute_pool_resume_data_from_bytes(
                ...
                pre_parsed_data={"name": "Pool Candidate", "email": "[email-redacted]"},
            )
        )
    ...
    async def _run() -> None:
        async with session_local() as session:
            ...
            emails_a = await resumes._existing_pool_emails(
                session, ["[email-redacted]"], owner_user_id="user-a"
            )
            emails_b = await resumes._existing_pool_emails(
                session, ["[email-redacted]"], owner_user_id="user-b"
            )

            assert "abc123" in hashes_a
            assert hashes_b == set()
            assert "[email-redacted]" in emails_a
            assert emails_b == set()
...
def test_assert_candidate_owner_blocks_cross_recruiter_access(tmp_path: Path) -> None:
    engine, session_local = _make_session_factory(tmp_path / "candidate_owner_scope.sqlite")
    ...
    async def _run() -> None:
        async with session_local() as session:
            ...
            pool_candidate = Candidate(
                id="owner-pool-candidate",
                job_id=None,
                user_id=owner.id,
                name="Pool Candidate",
                email="[email-redacted]",
            )
            session.add_all([owner, other, admin, job, job_candidate, pool_candidate])
            await session.commit()

            assert (await resumes._assert_candidate_owner(job_candidate.id, owner, session)).id == job_candidate.id
            assert (await resumes._assert_candidate_owner(pool_candidate.id, owner, session)).id == pool_candidate.id
            assert (await resumes._assert_candidate_owner(job_candidate.id, admin, session)).id == job_candidate.id

            with pytest.raises(HTTPException) as job_exc:
                await resumes._assert_candidate_owner(job_candidate.id, other, session)
            assert job_exc.value.status_code == 403

            with pytest.raises(HTTPException) as pool_exc:
                await resumes._assert_candidate_owner(pool_candidate.id, other, session)
            assert pool_exc.value.status_code == 403
...
@pytest.mark.parametrize(
    ("operation_name", "request_model"),
    [
        ("bulk_archive_candidates", resumes.BulkArchiveRequest),
        ("bulk_restore_candidates", resumes.BulkArchiveRequest),
        ("bulk_delete_candidates", resumes.BulkDeleteRequest),
    ],
)
@pytest.mark.parametrize("candidate_kind", ["pool", "job"])
def test_bulk_candidate_actions_reject_other_recruiter_candidates(
    tmp_path: Path,
    ...
    engine, session_local = _make_session_factory(tmp_path / f"{operation_name}_{candidate_kind}.sqlite")
    ...
    async def _run() -> None:
        async with session_local() as session:
            ...
            candidate = Candidate(
                id=f"other-{candidate_kind}-candidate",
                job_id=other_job.id if candidate_kind == "job" else None,
                user_id=None if candidate_kind == "job" else other.id,
                name="Other Recruiter Candidate",
                email=f"other.{candidate_kind}@example.com",
                is_archived=False,
            )
            session.add_all([user, other, other_job, candidate])
            await session.commit()
            ...
            operation = getattr(resumes, operation_name)
            operation_impl = getattr(operation, "__wrapped__", operation)
            with pytest.raises(HTTPException) as exc_info:
                await operation_impl(
                    request=SimpleNamespace(),
                    body=request_model(candidate_ids=[candidate.id]),
                    db=session,
                    user=user,
                )

            assert exc_info.value.status_code == 403
            still_present = await session.get(Candidate, candidate.id)
            assert still_present is not None
            assert still_present.is_archived is False
    ...
    async def _run() -> None:
        async with session_local() as session:
            ...
            async def _fake_parse(text: str, auth_header: str | None = None) -> dict:
                if "one.pdf" in text:
                    return {"name": "One", "email": "[email-redacted]"}
                return {"name": "Two", "email": "[email-redacted]"}
