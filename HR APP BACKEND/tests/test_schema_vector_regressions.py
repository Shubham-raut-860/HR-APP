...
def test_application_unique_index_blocks_duplicate_non_null_pair() -> None:
    ...
    with Session(engine) as session:
        ...
        first = Candidate(user_id=user.id, job_id=job.id, email="[email-redacted]")
        second = Candidate(user_id=user.id, job_id=job.id, email="[email-redacted]")
        session.add(first)
        session.commit()
...
def test_application_null_bearing_rows_insert_when_nullable() -> None:
    ...
    with Session(engine) as session:
        ...
        row_a = Candidate(user_id=user.id, job_id=None, email="[email-redacted]")
        row_b = Candidate(user_id=user.id, job_id=None, email="[email-redacted]")
